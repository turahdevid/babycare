...
export async function fetchTransactionDetailsPage(
  where: { gte: Date; lt: Date },
  extraWhere: {
    status?: string;
    midwifeId?: string;
...
): Promise<TransactionDetailPage> {
  const statusFilter = extraWhere.status
    ? { status: extraWhere.status as ReservationStatus }
    : {};
  const midwifeFilter = extraWhere.midwifeId ? { midwifeId: extraWhere.midwifeId } : {};
  ...
  const totalCount = await db.reservation.count({ where: reservationWhere });
  const pageCount = totalCount > 0 ? Math.ceil(totalCount / pagination.pageSize) : 0;

  const requestedPage = Number.isFinite(pagination.page) ? pagination.page : 1;
  const safePage = Math.max(1, Math.min(requestedPage, Math.max(pageCount, 1)));
