...
export default async function ChangePasswordPage({
  ...
  async function changePassword(formData: FormData) {
    ...
    if (!user?.password) {
      redirect("/account/password?error=unsupported");
    }
  ...
  return (
    <section className="grid gap-6">
      {showSuccess ? (
        <div className="rounded-[20px] border border-emerald-200/60 bg-emerald-50/40 p-4 text-sm font-medium text-emerald-800 shadow-[0_18px_50px_rgba(15,23,42,0.08)] backdrop-blur-2xl">
          Password berhasil diperbarui
        </div>
      ) : null}

      {showInvalid ? (
        <div className="rounded-[20px] border border-rose-200/60 bg-rose-50/40 p-4 text-sm font-medium text-rose-800 shadow-[0_18px_50px_rgba(15,23,42,0.08)] backdrop-blur-2xl">
          Password lama tidak valid
        </div>
      ) : null}

      {showUnsupported ? (
        <div className="rounded-[20px] border border-amber-200/60 bg-amber-50/40 p-4 text-sm font-medium text-amber-800 shadow-[0_18px_50px_rgba(15,23,42,0.08)] backdrop-blur-2xl">
